...
export default class {
  ...
  copySync(src, dest) {
    ...
    const body = current.getMeta().getBody();
    const destFile = this.findNode(dest);
    const { base: srcBase } = path.parse(src);
    const { base: destBase, dir: destDir } = path.parse(dest);

    if (!destFile) {
      const parent = this.findNode(destDir);
    ...
    if (destFile.getMeta().isDirectory()) {
      return destFile.addChild(srcBase, new File(srcBase, body));
    }

    return destFile.getParent().addChild(destBase, new File(destBase, body));
  }
  // Teacher's solution:
  // // BEGIN
  // copySync(src, dest) {
